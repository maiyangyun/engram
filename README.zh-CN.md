...
**有 Engram：**
> **你：** 把数据库建起来。
> **代理：** 用 PostgreSQL 建，因为这是你的偏好。要不要沿用上一个项目的 schema 模式？
...
| `org_id` | 组织范围——将可见性扩展到组织成员 | `"cortex"` |
| `project_id` | 项目范围——将可见性扩展到项目成员 | `"engram"`, `"imprint"` |

这些维度是**平行的，不是层级式的**。搜索时会匹配所有指定维度——未指定的维度视为通配符。

例如，用 `orgId="cortex"` 搜索会返回该组织的所有记忆，不限项目。加上 `projectId="engram"` 可以进一步缩小范围。没有组织/项目维度的记忆只有创建它的代理看得到。
...
Engram 由 **Ben**（AI）和 **Soren**（人类）共同打造，是 [Cortex](https://github.com/maiyangyun/engram#part-of-cortex) 项目的一部分。
